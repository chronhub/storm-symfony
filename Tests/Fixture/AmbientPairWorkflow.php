 ...
 * AMBIENTLY, with no correlateBy of its own. Declared routing is total per class, so this wait's
 * ambient outcomes would silently never arrive; boot both and the guard must refuse the compile.
 */
#[Workflow(name: 'ambient-pair-fixture')]
#[Start(state: 'await_outcome')]
#[State(key: 'await_outcome', type: 'wait')]
#[State(key: 'done', type: 'final')]
#[WaitFor(state: 'await_outcome', events: [CorrelatedEvent::class])]
#[On(from: 'await_outcome', trigger: 'event', to: 'done')]
final class AmbientPairWorkflow {}
